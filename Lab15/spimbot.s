...
main:
    ...
    puzzle_ready:
      ...
      la $a0, puzzle
    	...
    	la $a3, puzzle
    	sw $a3, SUBMIT_SOLUTION($0)
    	lw $t1, GET_PAINT_BUCKETS($0)
    	beq $t1, 50, start_paint        # 10
    	j request_puzzle
    ...
    start_paint:
      ...
      sw $t4, PICKUP_POWERUP
      la $t5, inventory
      sw $t5, GET_INVENTORY($0)
      li $t6, 0
      sw $t6, USE_POWERUP($0)

    	li $t0, 1
    	sw $t0, ENABLE_PAINT_BRUSH($0)		# enable paint brush
      li $t1, 0
      sw $t1, ANGLE_CONTROL
      li $t2, 119        # 119
      sw $t2, ANGLE
